...
For non-synthetic datasets which do not have specific similarities, we would build pseudo-synthetic datasets with similarities manually by following procedures. 
\begin{itemize}
  \item [1)] 
  ...
  i.e. If a new class is mixed by $k$ original labels, we sample a probability distribution $p$ of size $k$ and sum to 1. 
  So the sampling matrix is as below:
\begin{align}
    row &= (i+j) \bmod c\nonumber \\
    m[row,i] &= p[j]  \nonumber
\end{align}
where $i$=1,\dots,$o$ $j$=1,\dots,$k$ 
  
  \item [2)]
  We would sample new labels from the sampling matrix and view each row as embedding of each new label and then get similarities from embeddings.
  \item [3)]
  Next, we are able to construct a cover tree structure from the new labels and take each leaf node represents a new label and replace all parent nodes with pseudo labels. 
\end{itemize}
