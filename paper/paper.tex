...
\begin{abstract}
    %Multiclass logistic regression has generalization error $O(\sqrt{k/n})$, where $k$ is the number of classes and $n$ the number of data points.
...
%\begin{theorem}
    %Let $B,\rho>0$.
    %Let $f$ be a convex function and let $\star \w = \argmin_{\w : \ltwo{\w}\le B} f(\w)$.
    %Assume that SGD is run for $T$ iterations with $\eta = \sqrt{\frac{B^2}{\rho^2 T}}$.
    %Assume that for all $t$, $\ltwo{\nabla f(\w_t)} \le \rho$ with probability 1.
    %Then,
    %\begin{equation}
        %\E f(\bar \w) - f(\star \w) \le \frac{B\rho}{\sqrt{T}}
        %.
...
%It immediately follows that $\lF{W} \le \sqrt{k}B$,
...
%
%\begin{defn}
    %A learning problem is called convex-lipschitz-bounded with parameters $\rho,B$ if
    %\begin{enumerate}
        %\item $\ltwo{\w} \le B$
        %\item the loss
    %\end{enumerate}
%\end{defn}
%
%\begin{corollary}[Corollary 14.12 \citep{shalev2014understanding}]
    %Consider a convex-Lipschitz-bounded learning problem with parameters $\rho, B$.
    %Then for every $\epsilon>0$, if we run the SGD method for minimizing $L_\mathcal D(W)$ with a number of iterations
        %$T \ge \frac{B^2\rho^2}{\epsilon^2}$
    %and with
        %$\eta = \sqrt{\frac{B^2}{\rho^2 T}}$
    %then the output of SGD satisfies
    %\begin{equation}
        %\E L_\mathcal D (\bar W) - L_\mathcal D(\star W) \le \epsilon
    %\end{equation}
%\end{corollary}
%
%\subsection{Background}

%In this section, we first introduce more notation and definitions needed for our analysis,
%then we recall a standard result of stochastic gradient descent.
%We follow the notation of \citet{shalev2014understanding}'s textbook as closely as possible.
%
%Let $\mathcal D$ be a distribution over the input data points and $S \sim D^m$ be a set of $m$ data points sampled from this distribution.
%Then we define the true loss as
%\begin{equation}
    %L_\mathcal D(W) = \E_{(\x,y)\sim\mathcal D} \ell(W; \x, y)
%\end{equation}
%and the sample loss as
%\begin{equation}
    %L_S(W) = \frac 1 m \sum_{i=1}^m \ell(W; \x_i, y_i)
    %.
%\end{equation}
%We refer to the difference between these two losses as the generalization error.
%\begin{equation}
    %\left\lvert L_\mathcal D(W) - L_S(W) \right\rvert.
%\end{equation}
%
%Given a distribution of data points $\mathcal D$, the optimal parameter matrix is defined as
%\begin{equation}
    %\star W = \argmin_{W} \E_{(\x, y)} \ell(W; (\x, y))
%\end{equation}
%Given a dataset of $n$ points, we can estimate the parameter 
%\begin{equation}
    %\hat W = \argmin_{W} \frac1n\sum_{i=1}^n \ell(W; (\x_i, y_i))
%\end{equation}
%The main theoretical result for the cross entropy loss is that for a dataset with $n$ points,
%the generalization error is $O(\sqrt{k/n})$.
...
% A simple way to reduce sample complexity when $k$ is large is to factor the parameter matrix as $W = VU$,
...
% This factorization decreases the statistical complexity of the problem from $O(\sqrt{k/n})$ to $O(\sqrt{e/n})$.
% Recall that $e$ is a hyperparameter that is selected to be much less than $k$ and should represent the "intrinsic" dimensionality of the weight matrix.
% The following theorem proves this result formally.
% \begin{theorem}
% Assume that for all $i\in[n]$, $\ltwo{x_i} \le \rho$,
%     and that for each class $i\in[k]$, $\ltwo{\vv_i}\le 1$ and $\ltwo{\uu_i}\le B$.
...
%This cover tree will have exactly $k$ leaf nodes, one for each class label,
%and $O(k)$ internal nodes.
...
%which is significantly better than our bound on the weight matrix $W$ of only $\sqrt k B$.
...
%then
%\begin{equation}
    %\E L_D(\bar W) - L_D(W^*) \le \frac {\sqrt {\dim \mathcal L}B\rho\log k}{\sqrt T}
%\end{equation}
%where $W^* = \argmin L_D(W)$.
...
\begin{figure*}
        ...
        \begin{subfigure}[b]{0.45\textwidth}
            ...
            % \caption[Accuracy versus number of data points($d=64, \sigma=1.0, k=10$)]%
            {{\small Accuracy versus number of data points($d=64, \sigma=1.0, k=10$)}}  
            \label{subfig.1}
        \end{subfigure}
        \hfill
        \begin{subfigure}[b]{0.45\textwidth}  
            ...
            % \caption[Accuracy versus dimension of parameter matrix($n=100, \sigma=1.0, k=10$)]%
            {{\small Accuracy versus dimension of parameter matrix($n=100, \sigma=1.0, k=10$)}}
            \label{subfig.2}
        \end{subfigure}
        \vskip\baselineskip
        \begin{subfigure}[b]{0.45\textwidth}   
            ...
            % \caption[Accuracy versus randomness($n=100, d=64, k=10$)]%
            {{\small Accuracy versus randomness($n=100, d=64, k=10$)}}
            \label{subfig.3}
        \end{subfigure}
        \hfill
        \begin{subfigure}[b]{0.45\textwidth}   
...
\begin{figure*}
        ...
        \begin{subfigure}[b]{0.45\textwidth}
            ...
            {{\small Accuracy versus base of tree($n=1000, d=10, \sigma=1.0, k=100$)}}
            \label{subfig.1}
        \end{subfigure}
        \hfill
        \begin{subfigure}[b]{0.45\textwidth}  
            ...
            {{\small Height versus base of tree($n=1000, d=10, \sigma=1.0, k=100$)}}   
            \label{subfig.2}
        \end{subfigure}
        \vskip\baselineskip
        \begin{subfigure}[b]{0.45\textwidth}   
            ...
            % \caption[Accuracy versus randomness($n=100, d=64, k=10$)]%
            {{\small Accuracy versus tree structure($n=1000, d=64, \sigma=1.0, k=100$)}}  
            \label{subfig.3}
        \end{subfigure}
        \hfill
        % \begin{subfigure}[b]{0.45\textwidth}   
        ...
        %     % \caption[Accuracy versus number of classes($n=100, d=64, \sigma=1.0$)]%
        %     {{\small Loss versus base of cover tree($n=1000, d=10, \sigma=1.0, k=100$)}}    
        %     \label{subfig.4}
        % \end{subfigure}
        % \caption[ Figures of Synthetic Data Experiment ]
        % {{\small Figures of Synthetic Data Experiment}} 
