...
\end{lemma}
\begin{proof}
%Construct a cover tree over the distance metric $d$.
%For each class $i$, define the vector
%\begin{equation}
    %\uu_i =
    %\begin{cases}
        %\w_i & \text{if $i$ is the root of the cover tree} \\
        %\w_i - \w_{\parent{i}} & \text{otherwise} \\
    %\end{cases}
    %.
%\end{equation}
%Define the vector $\star\uu_i$ similarly by replacing all the $\w_i$ with $\star\w_i$ above.
...
\begin{table*}
    ...
    \caption{Experimental results on real world datasets.  For all accuracy measures, larger numbers are better.  The tree loss achieves the best results in all cases.}
    \label{table:results}
\end{table*}
