...
\begin{figure}[h]
    ...
    \caption{Loss value, parameter matrix error, accuracy versus number of data points}
    \label{Fig.1}
    ...
    \caption{Loss value, parameter matrix error, accuracy versus dimension of parameter matrix}
    \label{Fig.2}
    ...
    \caption{Loss value, parameter matrix error, accuracy versus randomness}
    \label{Fig.3}
...
The idea is we can sample the new classes from the original classes of datasets. We get the similarities of new classes from probability distribution and control the complexity of similarities by changing probability distribution. 
...
\begin{itemize}
  \item
  For $o$ original classes, we want to generate a sampling matrix $m$ for c new classes which each row represents the probability distribution of a new class. 
  Each new class is uniformly sampled from $k$ original class.
  So the sampling matrix is as below:
\begin{align}
    row &= (j+col) \bmod c \\
    m[i+row,col] &= \frac{o}{c \times n}  
\end{align}
where $i \in [c]$ $col \in [o]$ $j \in [k]$ 
  
  \item
  We would sample new classes from the sampling matrix and view each row as vector representation of each new class and then get similarities from vector representations.
  The new class is defined as below:
 \begin{align}
    c[i] = o[j] \cdot m[i,j] 
\end{align}
  \item
  Next, we are able to construct a cover tree structure from the new labels and take each leaf node represents a new class and replace all parent nodes with pseudo classes. 
\end{itemize}
