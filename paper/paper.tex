...
It treats every class equally while classes often have an internal structure. 
...
Assume that for all $i\in[n]$, $\ltwo{x_i} \le \rho$,
    and that for each class $i\in[c]$, $\ltwo{\w_i}\le \frac{B}{2^{\level k}}$.
Then the parameter vector $\bar W$ estimated from one epoch of SGD satisfies
\begin{equation}
...
If the class name include more than one word, we take a sum of the embedding of each word.

To evaluate our cover tree loss, we rely on two metrics: accuracy (Acc.) which focus on correct predictions and similarity accuracy (SA) which focus on the similarity of predicted and target class.
...
Cover Tree Loss                           & \textbf{53.44} & \textbf{66.7} & \textbf{53.5} & \textbf{65.89} \\
Cross Entropy                             & 52.08          & 65.46         & 51.87         & 65.58          \\
SimLoss                                   & 52.50           & 65.76         & 52.43         & 65.82          \\ \bottomrule
\end{tabular}
\end{center}
\end{table}
...
We use class names to get class representations from a pretrained BERT model \cite{Devlin2019BERTPO}.
We take ResNet50 model \cite{He2016DeepRL} and SGD optimizer with a learning rate 0.01.
Our metrics are top 1 accuracy and top 5 accuracy.


\begin{table}[h]
\small
\caption{Validation and test results on ImageNet dataset. Best values are written in bold.} \label{imagenet}
\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
\multicolumn{1}{c}{\multirow{2}{*}{Loss}} & \multicolumn{2}{c}{VALIDATION} \\ \cmidrule(l){2-3} 
\multicolumn{1}{c}{}                      & Top1 Acc.      & Top5 Acc.     \\ \cmidrule(r){1-1}
Cover Tree Loss                           &                &               \\
Cross Entropy                             &                &               \\
SimLoss                                   &                &               \\ \bottomrule
\end{tabular}
...
Our 1661 target emojis are defined by emo2vec \cite{Eisner2016emoji2vecLE}.
...
We get the embeddings of 1661 target emojis defined by emo2vec and construct a cover tree structure from these emojis. 
