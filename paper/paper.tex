...
\section{The Cover Tree Loss}

Of the previous techniques, only weight matrix factorization can provably reduce the statistical complexity of a problem.
The technique has two limitations, however:
(1) the class labels must have a linear structure and (2) the dimensionality of that structure must be known a priori.
The cover tree loss is able to take advantage of any metric structure and does not need to know the dimensionality in advance.

Like the weight matrix factorization, the cover tree loss works by reparameterizing the weight matrix for the softmax cross entropy loss.
The reparameterization is defined according to a tree structure that captures the similarity between labels.
Every class is represented by a leaf node in the tree,
and internal nodes represent \emph{pseudo-classes} that group the existing classes into a logical hierarchy.
The following figure shows an example tree.
...
We construct the loss function from this tree as follows.
...
That is, there is a vector $\vv_i$ for each class and for each pseudo-class.
...
\fixmemike{I need to add these results here}
